...
def func_chirp_mass(x, c):
    try:
        y = ((2/3)**(1/3) * c) / (np.sqrt(3) * np.sqrt(27 * c**2 * x**14 - 4 * c**3 * x**9) + 9 * c * x**7)**(1/3) + \
            (np.sqrt(3) * np.sqrt(27 * c**2 * x**14 - 4 * c**3 * x**9) + 9 * c * x**7)**(1/3) / (2**(1/3) * 3**(2/3) * x**3)
        return y
...
def generate_data(func, name, Xs, nXs, params, noises, oversample=False):
    ...
    for idx, param in enumerate(params):
        x = Xs[idx]
        y = func(x.T, *param)
        y = 10 * y / (max([abs(k) for k in y])) if np.abs(max([abs(k) for k in y])) != 0 else y
...
if __name__ == "__main__":  
    ...
    [[c] for c in c_values],  # Pass parameters. (I have choosen them so that their floor is 1 and 2 which we will fit our data to.)
    noises
)
